...
class TestConsulPlugin(unittest.TestCase):
    ...
    def setUp(self, mock_sink):
        ...
        default_regex = re.compile("|".join("(?:{0})".format(re.escape(x)) for x in consul_plugin.default_telemetry))
        self.plugin_conf = {
            "api_host": "example",
            "api_port": 8500,
            "api_protocol": "http",
        ...
        with patch("consul_plugin.ConsulAgent") as m_agent:
            ...
            self.plugin.global_dimensions.update(self.plugin.consul_agent.get_global_dimensions())

    def test_custom_dimensions(self):

        actual = self.plugin.global_dimensions
        expected = {"foo": "bar", "datacenter": "dc1", "consul_mode": "server", "consul_node": "server-3"}
    ...
    def test_fetch_server_state(self):
        ...
        expected_record = consul_plugin.MetricRecord("consul.is_leader", "gauge", 0, dimensions)
    ...
    def test_fetch_server_state_leader(self):
        ...
        expected_metric = consul_plugin.MetricRecord("consul.is_leader", "gauge", 1, expected_dim)

        actual_metric = self.plugin._fetch_server_state_leader()
        self.assertEqual(1, len(actual_metric))
    ...
    def test_fetch_peers(self):
        ...
        expected_metric = consul_plugin.MetricRecord("consul.peers", "gauge", 3, expected_dim)
    ...
    def test_fetch_catalog_metrics(self):
        ...
        expected_records = {
            "consul.catalog.services.total": [
                {"type": "gauge", "value": 2, "dimensions": self.plugin.global_dimensions}
            ],
            "consul.catalog.nodes.total": [{"type": "gauge", "value": 5, "dimensions": self.plugin.global_dimensions}],
        ...
        for (service, val) in service_to_nodes:
            ...
            del dim["consul_node"]
            metric = {"type": "gauge", "value": val, "dimensions": dim}
            expected_records["consul.catalog.nodes_by_service"].append(metric)

        node_to_services = [("client-1", 0), ("server-1", 1), ("server-2", 1), ("server-3", 1), ("google", 1)]
    ...
    def test_fetch_dc_network_metrics(self):
        ...
        expected_records.append(
            consul_plugin.MetricRecord("consul.network.dc.latency.avg", "gauge", 0.41374872289293463, dimensions)
        )
        expected_records.append(consul_plugin.MetricRecord("consul.network.dc.latency.min", "gauge", 0.02, dimensions))
        expected_records.append(
            consul_plugin.MetricRecord("consul.network.dc.latency.max", "gauge", 0.5702953069636298, dimensions)
        )
    ...
    def test_fetch_node_network_metrics(self):
        ...
        expected_records.append(
            consul_plugin.MetricRecord("consul.network.node.latency.avg", "gauge", 0.47051138518296787, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.network.node.latency.min", "gauge", 0.4154354798133847, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.network.node.latency.max", "gauge", 0.5571355904035926, dimensions)
        )
    ...
    def test_fetch_health_ckecks(self):
        ...
        expected_records = []
        dimensions = self.plugin.global_dimensions
        expected_records.append(consul_plugin.MetricRecord("consul.health.services.critical", "gauge", 0, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.health.services.warning", "gauge", 0, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.health.services.passing", "gauge", 0, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.health.nodes.critical", "gauge", 0, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.health.nodes.warning", "gauge", 0, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.health.nodes.passing", "gauge", 4, dimensions))
    ...
    def test_fetch_telemetry_metrics(self):
        ...
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.ip-10-2-2-84.ec2.internal.runtime.alloc_bytes", "gauge", 4117728, dimensions
            )
        )
    ...
    def test_fetch_telemetry_enhanced_metrics(self):
        ...
        expected_records = []
        dimensions = self.plugin.global_dimensions
        api_dimensions = dict(dimensions)
        api_dimensions.update({ "method": "GET", "path": "v1_agent_metrics" })

        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.consul.fsm.coordinate.batch-update.avg", "gauge", 0.05109499953687191, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.consul.fsm.coordinate.batch-update.max", "gauge", 0.05452900007367134, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.consul.fsm.coordinate.batch-update.min", "gauge", 0.04766099900007248, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.consul.http.GET.v1.coordinate.nodes.avg", "gauge", 0.7903540134429932, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.consul.http.GET.v1.coordinate.nodes.max", "gauge", 0.7903540134429932, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.consul.http.GET.v1.coordinate.nodes.min", "gauge", 0.7903540134429932, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.memberlist.gossip.avg", "gauge", 0.007076957123354077, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.memberlist.gossip.max", "gauge", 0.015080999583005905, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.memberlist.gossip.min", "gauge", 0.0043750000186264515, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.raft.fsm.apply.avg", "gauge", 0.10995149984955788, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.raft.fsm.apply.max", "gauge", 0.11391499638557434, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.raft.fsm.apply.min", "gauge", 0.10598800331354141, dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.ip-10-2-2-84.ec2.internal.consul.session_ttl.active", "gauge", 0, dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
        ...
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.ip-10-2-2-84.ec2.internal.runtime.free_count", "gauge", 124627230, dimensions
            )
        )
        expected_records.append(consul_plugin.MetricRecord("consul.memberlist.udp.received", "gauge", 2174, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.memberlist.udp.sent", "gauge", 2174, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.memberlist.tcp.sent", "gauge", 1458, dimensions))
        expected_records.append(consul_plugin.MetricRecord("consul.memberlist.tcp.accept", "gauge", 1, dimensions))
        expected_records.append(
            consul_plugin.MetricRecord("consul.api.http", "gauge", 6.331398844718933, api_dimensions
            )
        )
    ...
    def test_fetch_telemetry_enhanced_metrics_with_exclude(self):
        ...
        self.plugin.exclude_regex = re.compile("|".join("(?:{0})".format(re.escape(x)) for x in exclude))
    ...
    def test_metrics_excluded(self):
        self.plugin.enhanced_metrics = True
        exclude = ["consul.memberlist.gossip", "consul.ip-10-2-2-84.ec2", "consul.http.GET"]
        self.plugin.exclude_regex = re.compile("|".join("(?:{0})".format(re.escape(x)) for x in exclude))
        ...
        self.assertEqual(len(expected_metrics), len(actual_set))
    ...
    def test_telemetry_from_udp(self):
        ...
        expected_records = []
        expected_records.append(
            consul_plugin.MetricRecord("consul.memberlist.udp.received", "gauge", 181, self.plugin.global_dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.server-3.dc1.consul.runtime.alloc_bytes", "gauge", 4815376.0, self.plugin.global_dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord("consul.memberlist.gossip.min", "gauge", 0.006504, self.plugin.global_dimensions)
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.raft.rpc.appendEntries.max", "gauge", 0.014557, self.plugin.global_dimensions
            )
        )
        expected_records.append(
            consul_plugin.MetricRecord(
                "consul.server-3.dc1.consul.runtime.sys_bytes", "gauge", 13576440.0, self.plugin.global_dimensions
            )
        )
    ...
    def test_read_for_leader(self):
        self.plugin.read()
        self.assertEqual(24, len(self.plugin.metric_sink.captured_records))

    def test_read_for_leader_enhanced_mode(self):
        self.plugin.enhanced_metrics = True
        self.plugin.read()
        self.assertEqual(43, len(self.plugin.metric_sink.captured_records))
    ...
    def test_read_for_leader_enhanced_mode_with_exclude(self):
        ...
        self.plugin.exclude_regex = re.compile("|".join("(?:{0})".format(re.escape(x)) for x in exclude))
        self.plugin.read()
        self.assertEqual(36, len(self.plugin.metric_sink.captured_records))

    def test_read_for_leader_with_include(self):
        include = ["consul.memberlist"]
        self.plugin.include_regex = re.compile("|".join("(?:{0})".format(re.escape(x)) for x in include))
        self.plugin.read()
        self.assertEqual(31, len(self.plugin.metric_sink.captured_records))

    def test_read_for_not_leader(self):
        self.plugin.consul_agent.is_leader.return_value = False
        self.plugin.read()
        self.assertEqual(5, len(self.plugin.metric_sink.captured_records))

    def test_read_with_udp(self):
        self.plugin.udp_server = self._build_mock_udp_server()
        self.plugin.enable_server = True
        self.plugin.read()
        self.assertEqual(28, len(self.plugin.metric_sink.captured_records))
    ...
    def _validate_records_list(self, expected_records, actual_records):
        ...
        for record in actual_records:
            ...
            metric = {"value": record.value, "type": record.type, "dimensions": record.dimensions}
            actual_map[record.name] = actual_map.get(record.name, [])
            actual_map[record.name].append(metric)

        self.assertEqual(len(expected_records), len(actual_map))

        for k, v in list(actual_map.items()):
            self.assertCountEqual(expected_records[k], v)
    ...
    def _build_mock_consul_agent(self):
        ...
        sfx_token = None
        ssl_certs = {"ca_cert": None, "client_cert": None, "client_key": None}
        agent = consul_plugin.ConsulAgent(api_host, api_port, api_protocol, acl_token, sfx_token, ssl_certs)
        ...
        with patch("consul_plugin.ConsulAgent.get_nodes_in_dc") as n_call:
            with patch("consul_plugin.ConsulAgent.get_services_for_node") as s_call:
                ...
                mock_agent.get_catalog_map = MagicMock(return_value=catalog_map)

        with patch("consul_plugin.ConsulAgent.get_wan_coordinates") as mcall:
            mcall.return_value = self._sample_response("/coordinate/datacenters")
            dc_latency_map = agent.calculate_inter_dc_latency()
            mock_agent.calculate_inter_dc_latency = MagicMock(return_value=dc_latency_map)
        ...
        with patch("consul_plugin.ConsulAgent.get_lan_coordinates") as mcall:
            ...
            mock_agent.calculate_intra_dc_latency = MagicMock(return_value=node_latency_map)
    ...
    def _build_mock_udp_server(self):
        ...
        mock_server.stats = {
            "consul.memberlist.udp.received": {"timestamp": 1503336692.968225, "type": "gauge", "value": 181},
            "consul.server-3.dc1.consul.runtime.alloc_bytes": {
                "timestamp": 1503336692.96813,
                "type": "gauge",
                "value": 4815376.0,
            },
            "consul.memberlist.gossip.min": {"timestamp": 1503336692.968156, "type": "gauge", "value": 0.006504},
            "consul.raft.rpc.appendEntries.max": {"timestamp": 1503336692.968211, "type": "gauge", "value": 0.014557},
        ...
        for expected_record in expected_records:
            self.assertIsNotNone(
                next(
                    (
                        record
                        for record in self.mock_sink.captured_records
                        if self._compare_records(expected_record, record)
                    ),
                    None,
                ),
                "Captured record does not contain: {0} captured records: {1}".format(
                    expected_record.to_string(), [record.to_string() for record in self.mock_sink.captured_records]
                ),
            )
