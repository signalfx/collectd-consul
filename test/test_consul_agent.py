...
class ConsulAgentTest(unittest.TestCase):
    def setUp(self):
        ...
        self.ssl_certs = {"ca_cert": None, "client_cert": None, "client_key": None}
        self.agent = ConsulAgent(
            self.api_host, self.api_port, self.api_protocol, self.acl_token, self.sfx_token, self.ssl_certs
        )
    ...
    def test_return_json_on_ok(self, mock_urllib_opener):
        ...
        self.assertEqual(mock_urllib_opener.return_value.addheaders, expected_header)
    ...
    def test_send_leader_change_event_with_ok_status(self, mock_urlopen, mock_request):
        dimensions = {"foo": "bar"}
        ...
        mock_request.return_value.add_header.assert_has_calls(
            [mock.call("Content-Type", "application/json")], [mock.call("X-SF-TOKEN", self.agent.sfx_token)]
        )
    ...
    @mock.patch("consul_plugin.ConsulAgent.get_nodes_in_dc", return_value=sample_response("/catalog/nodes"))
    def test_get_catalog_map(self, mock_get_nodes, mock_get_services):
        ...
        expected_result = {
            "Nodes": {"client-1": 0, "server-1": 1, "server-2": 1, "server-3": 1, "google": 1},
            "Services": {"search": 1, "consul": 3},
        }
        actual_result = self.agent.get_catalog_map()
        self.assertDictEqual(actual_result, expected_result)

    @mock.patch(
        "consul_plugin.ConsulAgent.get_wan_coordinates", return_value=sample_response("/coordinate/datacenters")
    )
    def test_calculate_inter_dc_latency(self, mock_call):
        expected_dict_keys = {"dc2": {"avg": 0, "min": 0, "max": 0}}

        actual_response = self.agent.calculate_inter_dc_latency()
        self.assertIn("dc2", actual_response)
        self.assertEqual(set(expected_dict_keys["dc2"].keys()), set(actual_response["dc2"].keys()))

    @mock.patch("consul_plugin.ConsulAgent.get_lan_coordinates", return_value=sample_response("/coordinate/nodes"))
    def test_calculate_intra_dc_latency(self, mock_call):
        expected_dict_keys = {"avg": 0, "min": 0, "max": 0}
        actual_response = self.agent.calculate_intra_dc_latency()
        self.assertEqual(set(expected_dict_keys.keys()), set(actual_response.keys()))

    @mock.patch("consul_plugin.ConsulAgent.get_health_checks", return_value=sample_response("/health/state/any"))
    def test_get_health_check_stats(self, mock_call):
        expected_response = {"service": {}, "node": {"passing": 4}}

        actual_response = self.agent.get_health_check_stats()
        self.assertEqual(expected_response, actual_response)

    def test_get_global_dimensions(self):
        expected_response = {"datacenter": "dc1", "consul_mode": "server", "consul_node": "server-3"}
