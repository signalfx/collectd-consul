...
class MetricRecord(object):
    ...
    def to_string(self):
        return MetricRecord.TO_STRING_FORMAT.format(
            self.name, self.type, self.value, self.instance_id, self.dimensions, self.timestamp
        )
...
class MetricSink(object):
    ...
    def emit(self, metric_record):
        ...
        emit_value.plugin_instance = "[{0}]".format(_format_dimensions(metric_record.dimensions))
...
class UDPServer(threading.Thread):
    ...
    def run(self):
        ...
        LOGGER.info(
            "Starting UDP server on {0}:{1} "
            "to collect Consul's telemetry data. Timeout set to {2}.".format(self._host, self._port, self._timeout)
        )
        ...
        try:
            ...
            while not self.terminate.isSet():
                try:
                    ...
                    LOGGER.warn(
                        "UDP Server timed out waiting for " "telemetry data on {0}:{1}.".format(self._host, self._port)
                    )
                    continue
        ...
        for datapoint in data.splitlines():
            ...
            if metric_split[1] == "c" and metric_name in self.metrics:
                self.metrics[metric_name] = {
                    "type": metric_type,
                    "value": self.metrics[metric_name]["value"] + int(float(metric_split[0])),
                    "timestamp": time.time(),
                }
                ...
                self.metrics[metric_name] = {"type": metric_type, "value": val, "timestamp": time.time()}
    ...
    def _include_metric(self, metric_name):
        if self.default_regex.search(metric_name) is not None:
            ...
            if self.exclude_regex is None or (
                self.exclude_regex is not None and self.exclude_regex.match(metric_name) is None
            ):
                return True
        elif self.include_regex is not None and self.include_regex.match(metric_name) is not None:
            return True
        return False
...
def configure_callback(conf):
    ...
    include_metrics = []
    custom_dimensions = {}
    default_telemetry_regex = re.compile("|".join("(?:{0})".format(re.escape(x)) for x in default_telemetry))
    ...
    if exclude_metrics:
        exclude_metrics_regex = re.compile("|".join("(?:{0})".format(x) for x in exclude_metrics))
    else:
        exclude_metrics_regex = None

    if include_metrics:
        include_metrics_regex = re.compile("|".join("(?:{0})".format(x) for x in include_metrics))
    else:
    ...
    for k, v in list(plugin_conf.items()):
    ...
    collectd.register_read(consul_plugin.read, name="{0}:{1}".format(api_host, api_port))
...
class ConsulPlugin(object):
    def __init__(self, plugin_conf):
        ...
        self.consul_agent = ConsulAgent(
            plugin_conf["api_host"],
    ...
    def _fetch_server_state(self):
        ...
        if self.consul_agent.config.get("Config", {}).get("Server", False):
            ...
            metric_records.append(
                MetricRecord(consul_server_state.name, consul_server_state.type, 0, dimensions, time.time())
            )
    ...
    def _fetch_server_state_leader(self):
        ...
        dimensions.update(self.global_dimensions)
        metric_records.append(
            MetricRecord(consul_server_state.name, consul_server_state.type, 1, dimensions, time.time())
        )
    ...
    def _fetch_peers(self):
        ...
        if peers:
            metric_records.append(
                MetricRecord(consul_peers.name, consul_peers.type, peers, self.global_dimensions, time.time())
            )
    ...
    def _fetch_catalog_metrics(self):
        ...
        metric_records.append(
            MetricRecord(
                catalog_services.name, catalog_services.type, total_services, self.global_dimensions, time.time()
            )
        )
        metric_records.append(
            MetricRecord(catalog_nodes.name, catalog_nodes.type, total_nodes, self.global_dimensions, time.time())
        )

        for service, num_nodes in catalog_map["Services"].items():
            dimensions = {"consul_service": service}
            dimensions.update(self.global_dimensions)
            del dimensions["consul_node"]
            metric_records.append(
                MetricRecord(
                    catalog_nodes_by_service.name, catalog_nodes_by_service.type, num_nodes, dimensions, time.time()
                )
            )

        for node, num_services in catalog_map["Nodes"].items():
            dimensions = {}
            dimensions.update(self.global_dimensions)
            dimensions["consul_node"] = node
            metric_records.append(
                MetricRecord(
                    catalog_services_by_node.name, catalog_services_by_node.type, num_services, dimensions, time.time()
                )
            )
    ...
    def _fetch_dc_network_metrics(self):
        ...
        for dest_dc, metrics in latencies.items():
            dimensions = {"destination_dc": dest_dc}
            dimensions.update(self.global_dimensions)
            metric_records.append(
                MetricRecord(
                    dc_network_latency_avg.name, dc_network_latency_avg.type, metrics["avg"], dimensions, time.time()
                )
            )
            metric_records.append(
                MetricRecord(
                    dc_network_latency_min.name, dc_network_latency_min.type, metrics["min"], dimensions, time.time()
                )
            )
            metric_records.append(
                MetricRecord(
                    dc_network_latency_max.name, dc_network_latency_max.type, metrics["max"], dimensions, time.time()
                )
            )
    ...
    def _fetch_telemetry_metrics(self):
        ...
        if self.enable_server:
            ...
            with self.udp_server.lock:
                for metric_name, stat in list(self.udp_server.stats.items()):
                    metric_records.append(
                        MetricRecord(
                            metric_name, stat["type"], stat["value"], self.global_dimensions, stat["timestamp"]
                        )
                    )
            ...
            for metric_type, metrics_list in list(metrics.items()):
                if metric_type == "Timestamp":
                    continue
                for metric in metrics_list:
                    if self._include_metric(metric["Name"]):
                        metric_records.extend(self._sanitize_telemetry(metric_type, metric))
...
class ConsulAgent(object):
    ...
    def __init__(self, api_host, api_port, api_protocol, acl_token, sfx_token, ssl_certs):
    ...
    def get_services_for_node(self, node):
        return self._send_request("{0}/{1}".format(self.list_services_for_node_url, node))
    ...
    def is_leader(self):
        ...
        if curr_leader == agent_addr:
            if self.last_leader is not None and self.last_leader != curr_leader and self.sfx_token:

                LOGGER.debug("Change in leader.")
                dimensions = {
                    "old_leader": self.last_leader.split(":")[0],
    ...
    def _send_leader_change_event(self, dimensions):
        ...
        try:
            req = urllib.request.Request(self._event_url, payload)
            for k, v in list(sfx_headers.items()):
                req.add_header(k, v)
            response = urllib.request.urlopen(req)
            LOGGER.debug("Sent leader change event to SignalFX with " "response code {0}".format(response.code))
        except urllib.error.HTTPError as e:
            LOGGER.error("HTTPError - status code: {0}, " "received from {1}".format(e.code, self._event_url))
    ...
    def _send_request(self, url):
        ...
        try:
            ...
            opener = urllib.request.build_opener(
                urllib_ssl_handler.HTTPSHandler(
